...
def custom_loss(Pl4, target, weights):
    ...
    return F.binary_cross_entropy(Pl4, target[4].float(), weight=weights[4].float())
...
if __name__ == '__main__':
    ...
    plt.title("MultiLabelNN, Results at Testing across Epochs%s%s." % (", Similarity-Weighted" if args.similarity else "", ", Inhibitors" if args.inhibitors else ""))
    plt.xlabel("Epochs")
    plt.grid()
    plt.legend()
    plt.savefig(os.path.join(output_folder, "MultiLabelNN_metrics_by_epoch%s%s_best_drop%.2f_layer%d.pdf" % (sim, inh, best_dropout, best_layer)))

    pickle_dump(rocs_list, os.path.join(output_folder, "MultiLabelNN_rocs_list%s%s_best_drop%.2f._layer%dpkl" % (sim, inh, best_dropout, best_layer)))
    pickle_dump(PRs_list, os.path.join(output_folder, "MultiLabelNN_PRs_list%s%s_best_drop%.2f_layer%d.pkl" % (sim, inh, best_dropout, best_layer)))
    pickle_dump(R_precs_list, os.path.join(output_folder, "MultiLabelNN_R_precs_list%s%s_best_drop%.2f_layer%d.pkl" % (sim, inh, best_dropout, best_layer)))
    pickle_dump(results_list, os.path.join(output_folder, "MultiLabelNN_results_list%s%s_best_drop%.2f_layer%d.pkl" % (sim, inh, best_dropout, best_layer)))
    pickle_dump(targets_list, os.path.join(output_folder, "MultiLabelNN_targets_list%s%s_best_drop%.2f_layer%d.pkl" % (sim, inh, best_dropout, best_layer)))